...
class CaptioningRNN(object):
  ...
  def sample(self, features, max_length=30):
    ...
    capt_cur = self._start *np.ones((N, 1), dtype = np.int32)
    for t in range(max_length):
        word_embed, cache_embed = word_embedding_forward(capt_cur, W_embed)
        if self.cell_type == 'rnn':
            h, cache_rnn = rnn_step_forward(np.squeeze(word_embed), prev_h, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            h, c, cache_lstm = lstm_step_forward(np.squeeze(word_embed), prev_h, prev_c, Wx, Wh, b)
