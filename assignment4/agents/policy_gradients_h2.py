...
class PolicyGradient_h2(object):
    ...
    (Pong pixels) - (FC layer) - (ReLU) - (FC layer) -(ReLU) - (FC layer) - (Sigmoid)
    ...
    def __init__(self, D=4, H1=20, H2 = 20, learning_rate=1e-2, batch_size=10, gamma=0.99, 
        ...
        self.model = {}
        self.model['W1'] = np.random.randn(H1,D) / np.sqrt(D)
        self.model['W2'] = np.random.randn(H2,H1) / np.sqrt(H1)
        self.model['W3'] = np.random.randn(H2) / np.sqrt(H2)
    ...
    def policy_forward(self, x):
        ...
        h2[h2<0] = 0
        output = np.dot(self.model['W3'], h2)
    ...
    def discount_rewards(self, r, do_reset=False):
        ...
        for t in reversed(xrange(0, r.size)):
            if do_reset:
                if r[t] > 1e-5 or r[t] < -1 * 1e-5:
                    r_sum = 0.0
            
            r_sum = r_sum  * self.gamma + r[t]
            discounted_r[t] = r_sum 
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################
    ...
    def policy_backward(self, ep_x, ep_h1, ep_h2, ep_dprobs, discounted_epr):
        ...
        dW3 = np.dot(ep_h2.T, ep_dprobs).ravel()
        
        #print dW2
        dh2 = np.outer(ep_dprobs, self.model['W3'])


        dh2[ep_h2 <= 0] = 0 
        dW2 = np.dot(dh2.T, ep_h1)
        #print dW2
        dh1 = np.dot(ep_h2, self.model['W2'])
        dh1[ep_h1 <= 0] = 0 
        dW1 = np.dot(dh1.T, ep_x)
        # print "====="
        # print self.model['W1'].shape
        # print self.model['W2'].shape
        # print self.model['W3'].shape
        # print dW1.shape
        # print dW2.shape
        # print dW3.shape
        ########################################################################
    ...
    def train(self, environment="CartPole-v0", max_episodes=100, print_every=1):
        ...
        while True:
            ...
            if environment == "CartPole-v0":
                ...
                x = observation
                aprob, h1, h2, = self.policy_forward(x)
            ...
            if done:
                ...
                grad = self.policy_backward(ep_x, ep_h1, ep_h2, ep_dprobs, discounted_epr)
                for w in self.model:
                    self.grad_buffer[w] -= grad[w]
